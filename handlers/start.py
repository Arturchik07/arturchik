    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(text="Наш сайт", url="https://www.wasabi.kg")
            ],
            [
...
async def our_address_handler(callback: CallbackQuery):
    await callback.message.answer("боконбаева 183")

async def contacts_handler(callback: CallbackQuery):
    await callback.message.answer("Наши контакты 032343422")

async def time_work_handler(callback: CallbackQuery):
    await callback.message.answer("С 8:00 am до 2:00 pm")
